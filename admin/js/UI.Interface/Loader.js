...
function WATMessage() {
  ...
  this.view = function (params) {
    ...
    var no_messages = jQuery('div[id="watMessageBox"]', this.parentContainer).get().length;

    // add message box to the parent container
    var html = '<div id="watMessageBox" class="loader" style="display:none;">';
    html += '<div id="watBoxTable" align="left" style="background: #fefcd4;">';
    html += '<div style="float:right; height:15px; padding-right:5px; padding-top:5px"><img id="watCloseMsg" src="' + WATJSInterface.localpath + 'admin/images/btn_close_msg.png" border="0" style="cursor:pointer" /></div>';
    ...
    this.container = jQuery('div[id="watMessageBox"]:eq(' + no_messages + ')', this.parentContainer);
    var table = jQuery('#watBoxTable', this.container);
    var close_btn = jQuery('#watCloseMsg', table);
...
function WATLoader() {
  ...
  this.init = function () {
    ...
    jQuery('#wat_messages_container').remove();
    jQuery(jQuery('body').get(0)).append('<div align="center" id="wat_messages_container" style="position:fixed; z-index:999999; width: 100%; display:block; top: 5px; margin: 0 auto;"></div>');

    var messages_container = jQuery('#wat_messages_container');
